...
const RecommendUserService = class RecommendUserService {
   ...
   */
  getTargetGoods() {
    // 截至目前，以获取用户相似度的逆序排序数组，
    // 以下为获取前n个相似用户购买的所有商品中本用户没买过的
  ...
  duplicateRemovalGoods() {
    // 获取当前用户买过的商品
    const userGoods = this.data.filter((obj) => {
  ...
  getUserSimilarity(userId, otherUserId) {
    const userSelfGoods = this.data.filter((obj) => {
  ...
  filterUserGoods(otherUserId) {
    this.userGoodsTemp = this.data.filter((obj) => {
  ...
  filterGoodsById(goodsId) {
    const goods = this.data.filter((obj) => {
